...
def model_builder(hp):
  ...
  hp_units = 43#hp.Int('units', min_value=1, max_value=100, step=1)
#  for i in range(hp.Int('n_layers', 1, 10)):
#    hp_act = hp.Choice(
#                    f'dense_activation_{str(i)}',
#                    values=['relu', 'gelu', 'selu', 'leaky_relu', 'mish'],
#                    default='relu')
  model.add(tf.keras.layers.Conv2D(hp_units,
                                   (5, 5),
                                   activation='gelu', kernel_initializer='lecun_normal', input_shape=(28, 28, 1)))
  
  model.add(tf.keras.layers.Conv2D(hp_units,
                                   (3, 3),
                                   activation='relu', kernel_initializer='lecun_normal', input_shape=(28, 28, 1)))
